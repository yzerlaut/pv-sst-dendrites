...
def sim_release_pretty(release_proba_params={},
                events = np.concatenate([0.1+np.arange(7)/10., [1.4]]),
                tstop=1.3, nTrials=10,
                dt = 5e-4,
                seed=0,
                with_title=True, figsize=(.95,1.2)):
    ...
    Rs, t = [], np.arange(tstop/dt)*dt
    for i in range(nTrials):
        AX[0].scatter(pre_Events[i], space*i*np.ones(len(pre_Events[i])), facecolor='k', edgecolor=None, alpha=.35, lw=0, s=6)
        AX[1].scatter(release_Events[i], space*i*np.ones(len(release_Events[i])), facecolor='k', edgecolor=None, alpha=.35, lw=0, s=10)
        Rs.append(rough_release_dynamics(release_Events[i], tstop=tstop, dt=dt))
        AX[2].plot(t, space*i+Rs[-1], color='dimgrey', lw=0.5, alpha=0.7)
    # we add some more
    for i in range(20*nTrials):
        release_Events = get_release_events(pre_Events[i%nTrials], **release_proba_params)
        Rs.append(rough_release_dynamics(release_Events, tstop=tstop, dt=dt))
    
    AX[3].plot(t, np.mean(Rs, axis=0), 'k-', lw=1)
    
    for ax in AX:
        ax.plot([0,0.05], (nTrials+0.5)*np.ones(2), 'k-', lw=1)
        pt.set_plot(ax, [], ylim=[-0.5,space*(nTrials+3)], xlim=[.05,tstop-.05])
...
fig = sim_release_pretty(events=0.1+np.arange(9)/20., tstop=0.65, release_proba_params=PV_model, seed=4)
fig.savefig('../figures/Temp-Properties-Pred/STP_PV.svg')
...
fig.savefig('../figures/Temp-Properties-Pred/STP_SST.svg')
...
# %%
def get_rate(t, duration=0.5, stimFreq=1., stepFactor=3.):
    rate = stimFreq+0*t
    rate[(t>(t[-1]/2.-duration/2.)) & (t<(t[-1]/2.+duration/2.))] *=stepFactor
    return rate 
    
def sim_release(release_proba_params={},
                stimFreq = 1., stepFactor=3.,
                nSyns = 200, tau=0.02,
                tstop=1.5, duration=0.5,
                dt = 1e-3):

    t = np.arange(int(tstop/dt))*dt
    rate = get_rate(t, duration=duration, stimFreq=stimFreq, stepFactor=stepFactor)
...
# %%
fig, AX = pt.figure(axes=(4,5), figsize=(1,0.8), hspace=0.2, wspace=0.2)
...
for i, freq in enumerate([0.5, 1, 1.5, 2, 2.5]):
    for j, duration in enumerate([0.05, 0.2, 1., 2.]):
        ...
        if j==0:
            pt.annotate(AX[i][0], 'f=%.1fHz' % freq, (-0.3, 0.5), ha='center', rotation=90)
        if i==0:
            pt.annotate(AX[0][j], 'dur.=%.2fs' % duration, (0.5, 1.1), ha='center')
...
for i, freq in enumerate([2, 4, 6, 8, 10]):
    for j, duration in enumerate([0.05, 0.2, 1., 2.]):
        ...
        if i==0:
            pt.annotate(AX[0][j], 'dur.=%.2fs' % duration, (0.5, 1.1), ha='center')
        AX[i][j].set_xlim(t[-1]/2.-duration, t[-1]/2.+duration)
        AX[i][j].axis('off')
        pt.draw_bar_scales(AX[i][j], Xbar=0.5, Ybar=5e-3)
...
SIMS = {\
    'models':['PV', 'PV-no-STP', 'SST', 'SST-no-STP'],
    'PV':   np.load('../data/detailed_model/PV_stp.npy', allow_pickle=True).item(),
    'PV-no-STP':  {'P0':0.8, 'P1':0.8, 'dP':0.00, 'tauP':1.0, 'Nmax':1},
    'SST':   np.load('../data/detailed_model/SST_stp.npy', allow_pickle=True).item(),

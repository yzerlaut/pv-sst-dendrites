...
def compute_responses(data,
    ...
    for roi in np.arange(data.nROIs):                                                                                                                                                                                                         
        ...
        if np.sum(cell_resp['significant'][full_contrast_cond]):

            RESPONSES['roi'].append(roi)
            ipref = np.argmax(cell_resp['value'][full_contrast_cond])
            prefered_angle = cell_resp['angle'][full_contrast_cond][ipref]
